...
(defn instagram-2019-11
  []
  (let [small-radius 10
        shatter-padding 5
        n-rands (fn [n]
                  (sort (repeatedly n rand)))
        circle-radius (-> INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
                          (* (/ 7 8))
                          (* (/ 1 2))
                          ->int)
        gap (->int (/ (- INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
                         (* circle-radius 2))
                      2))
        center (->int (/ INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
                         2))
        circle {:x center
                :y center
                 ...
                 (let [n-interior-lines (* 2 idx_1_based)
                       ...
                       ->interior-lines (fn [endpoint-ys center-points beziers]
                                          (map
                                            (fn [endpoint-y center-y [b0 b1] [b3 b2]]
                                              (->interior-line [{:x 0
                                                                 :y endpoint-y}
                                                                {:x INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
                                                                 :y endpoint-y}]
                   ...
                   (render [year month day]
                           (shatter/dark
                             ...
                             shatter-padding)
                           (format ""))))]
    (doseq [[idx_1_based year month day] (indexed-days-of-week (week->date 2019 11))]
      (gen-fn idx_1_based year month day))))
...
(comment
  ...
  (trace/profile {} (instagram-2019-11)))
