    ...
    (try
      ...
      (spit (str filename ".txt")
            (format-w-newlines
              [[description]
               ["Link to code in bio."
                "#code #art #codeart #codedesign #codeartist"
                "#programming #creativecoding #daily"
                "#codexart #generativeart #mathart"
                "#algorithmicart #generativecoding #processing"]]))
      (catch Error e
        (log/error e)
  ...
  (let [gen-fn (fn [idx_1_based year month day]
                 (let [pipe-endpoints (inc (rand-int 3))
                       ...
                       grid-size (int (/ INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
                                         day pipe-endpoints))
                       description (format-w-newlines
                                     [["Generated using %s x %s tiles to represent the day of the month."
  ...
  (let [colours (->> (concat ink.cc/web-safe-colors
        ...
        gen-fn (fn [idx_1_based year month day]
                 (let [height-width (-> (/ INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
                       ...
                       description (format-w-newlines
                                     [["Generated using %s x %s tiles to represent the day of the week"
  ...
  (let [png-swatches (->> "./swatches/png/nature"
        ...
        gen-fn (fn [idx_1_based year month day svg-swatch]
                 (let [hist-path (transform/shuffle svg-swatch)
                       ...
                       description (format-w-newlines
                                     [["Generated using selected source images, raw is present in the top left corner."]
  ...
  (let [c (int (/ INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
        ...
        gen-fn (fn [[tag attrs] idx_1_based year month day]
                 (render [year month day]
                         ...
                         (format-w-newlines
                           [["A shape which has %s points. The number"
                                                  ...
                                                  (fn []
                                                    [(+ (rand-int control-width-height)
                                                     ...
                                                     (+ (rand-int control-width-height)
                                                        control-padding)]))
                       description (format-w-newlines
                                     [["Generated using %s random points to"
                                       "represent the day of the week (ie, 1 for Monday, 2 for Tuesday, etc.)."
        ...
        gen-fn (fn [idx_1_based year month day]
                 (let [starting-dimension (->> starting-dimensions
                   ...
                   (render [year month day]
                           hiccup-svg
                           (format-w-newlines
                             [["Generated using %s (day of the month) steps. Each step consists of"
                 ...
                 (let [xn 20
                   ...
                   (render [year month day]
                           (tile/grid
                             ...
                             xn 4)
                           (format-w-newlines
                             [["The first row of colours is filled 20, the second is filled 19..."
        ...
        gen-fn (fn [idx_1_based year month day]
                 (let [shape (layer/infinite
                   ...
                   (render [year month day]
                           ...
                           (format-w-newlines
                             [["%s (day) cubes, randomly skewed, infinitely rotated about"
                               "center."]
  ...
  (let [line-control-colour "darkslategray"
        ...
        gen-fn (fn [idx_1_based year month day]
                 (render [year month day]
                         ...
                         (format-w-newlines
                           [["Playing around with gradients this week. There are %s by %s tiles, where"
                           ...
                           [:svg {:width INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
                            ...
                            [:circle {:cx (/ INSTAGRAM-RECOMMENDED-MIN-WIDTH-HEIGHT
                                      ...
                                      :style (format "fill:%s;stroke:%s;stroke-width:%s"
                                                     ...
                                                     10)}]]
                           (format-w-newlines
                             [["%s by %s tiles for the day of the month."
